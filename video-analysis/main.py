...
async def main(message: cl.Message):
    ...
    video_elements = [
        elem for elem in message.elements if elem.mime.startswith("video/")
    ]

    if not video_elements:
        await cl.Message(content="Please provide a video to analyze.").send()
        return
        ...
        try:
            ...
            for i, frame in enumerate(frames):
                ...
                await cl.Message(
                    ...
                    elements=[
                        cl.Image(name=f"Frame {i+1}", content=frame, display="inline")
                    ],
                ).send()
